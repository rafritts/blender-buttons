...
_Status: **SUPERSEDED (2026-06-19) by [SPEC-12](SPEC-12-collab-panel.md).** V1 was
implemented and live-validated — all four success criteria passed (round-trip,
continuity, the 10cm-sphere proof demo, latency), and the agent drove geometry
verbs in the same loop. But the dogfood surfaced that conversation was never the
gap: the terminal handles talking fine; what was missing was **shared state**. The
chat code (`extension/chat.py`, `server/verbs/chat.py`, the chat panel / Send
operator / drain timer / `bb_chat_input` prop, the `chat` verb + its state-set
exemptions) has been REMOVED and replaced by the SPEC-12 Collab panel (phase +
sign-off queue + handles, with decisions flowing back on demand via `collab
op=status` instead of a long-poll loop). This doc is kept as a documented dead-end —
the plumbing insight (carry collaboration as ordinary tool I/O over the existing
socket) carried forward; the chat surface did not. Original spec text below._
