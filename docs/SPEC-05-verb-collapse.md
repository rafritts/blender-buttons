...
## Addendum C — Second dogfooding pass (2026-06-15)

A second live run on the Spring rig (strip scarf / jacket / pullover-mask, cut the pullover
down to a tank top — the sleeve removal this time) surfaced five more. **#1–#4 are fixed in
this pass** (verified headless: `tests/e2e_addendumC.py` + the updated Z3/Z3b in
`tests/e2e_batch10.py`). **#5 is deferred** — it is not a bug to patch but a design direction
(below the table).

The connective tissue: every fix lands on a mechanism SPEC-05 already owns — the auto
mode-switch (§Auto mode-switching), the generalized warning/notes channel (§Improvements #4),
the self-correcting-enum-error pattern (Addendum B #3), and Blender-native `extend`.

| # | Severity | Symptom | Root cause | Fix |
|---|---|---|---|---|
| 1 | **high** | `select(op="object", name=…)` from POSE mode failed with the raw `bpy.ops.object.select_all.poll() failed, context is incorrect` — no hint, no switch. A new user is simply stuck. | SPEC-05's auto mode-switch declares **one mode per verb**, but `select` is **bimodal**: object ops (`object`/`all`/`none`) need Object Mode, component ops (`by_axis`/`ring`/…) need Edit Mode. Only "select → Edit" was wired, so the object path never left POSE/SCULPT and hit the operator poll. | `extension/objects.py:select_object` now ensures Object Mode first (the verb *is* the mode context) and reports the switch through a new generalized **`notes`** channel surfaced in the status block (`server/_core.py:_status`) — realizing §Improvements #4. The raw poll error is gone; the agent is told "auto-switched POSE → OBJECT". `transform` is the same bimodal shape and should get per-op mode declaration as it grows. |
| 2 | **schema/doc** | `feel(op="topology", method="openings,branches")` → `unknown method`. The valid tokens were undiscoverable. | The **verb** docstring (`feel.py`) — the only one the agent reads — described topology's *outputs* in prose ("openings, branches, hard edges, cross-sections") which **read like method tokens but aren't**. The real tokens (`boundaries`, `sections`, `features`, …) lived only in the pruned flat tool's docstring. | Verb docstring + the `method` `tag(...)` now enumerate the **real tokens** (and map the concept-words to them, e.g. boundaries = "the openings"). An unknown method now returns a **self-correcting** error listing every valid token (`extension/topology.py`), reusing the Addendum B #3 pattern. |
| 3 | **defect** | `feel(op="profile")` dumped **200 raw rings** with a width column that was almost all `0.0000` — noise, not the "where does the section pinch" answer the tool exists to give. It also flooded context. | `get_mesh_profile` grouped verts by **exact** axis position (`round(pos,4)`), so most "rings" held one vert → zero width. No aggregation, no feature detection. | Default now **aggregates into evenly spaced bands** (`bands=24`), each reporting its real cross-section width (span across all its verts), with the **narrowest band (the pinch) and widest flagged**. The raw per-ring dump — and its windowing / `max_rings` resampling — is opt-in via **`full=True`** (`extension/objects.py`, `server/queries.py`, `feel.py`). |
| 4 | **defect** | `select(op="by_axis", action="SELECT")` called twice **replaced** the selection each time instead of unioning, so two regions (both sleeves) could never be selected before one delete. The count *looked* plausible (460, not 920) → a silent-wrong-result trap. | The `action` enum had no `EXTEND`; `SELECT` always cleared non-matching verts first. (`select_in_sphere` quietly had an `ADD` action, but it was unexposed and inconsistent.) | Added a Blender-native **`extend`** bool to `by_axis` / `between` / `in_sphere` (the region selects), threaded through verb → server → engine; `extend=True` unions onto the current selection. `in_sphere` is unified onto its existing `ADD` path. Default stays replace (predictable). |
| 5 | **design direction (deferred)** | `select(op="by_axis", factor=…)` normalizes `factor` against **live** bounds, so after an edit shrinks the mesh the *same* intent needs a *different* number (`0.683 → 0.535` to keep cutting at x≈0.16). | `factor` is relative-to-current-extent; the relativity silently re-anchors as geometry changes. | **Not patched — and deliberately not "fixed" with an absolute world-coordinate input.** Feeding a remembered world coord back is dead reckoning (`GUIDANCE_FOR_LLMS.md` §"the one rule": coordinate math only survives in a self-authored, origin-centered, just-now regime — it fails on big/offset/edited scenes). The right direction is **feature-anchored selection**: the armhole is a *pinch* `feel` already detects (Addendum C #3) sitting between two boundary loops `feel` already enumerates. `select` should act relative to that structure — "the sleeve outboard of the pinch ring" — so no coordinate is ever typed. The mesh becomes the coordinate system; the read→act loop closes in the mesh's own terms. Tracked for a dedicated pass. |
