...
- **P9 — `check_framing(camera?, targets?)` — camera-space report.** Project target bboxes through the camera matrix and report frame coverage, clipped edges, and occlusion: `"watch: 71% of frame width, bottom edge clipped by 4% — crown: 80% occluded by bezel from this angle"`. Pure matrix math + a handful of ray casts. On the pocket-watch build, 3 of 5 test Cycles renders existed mainly to answer "is it still cropped?" — each ~45s + image tokens for a question with an exact deterministic answer.
- **P10 — `check_resting(targets?)` — gravity sanity.** For each part: contact points with the floor/supporting object, penetration depth, float height, and whether the center of mass sits over the support region (`"bow: 1 contact point, COM 60mm outside support — would tip toward +Y"`). The watch bow's tilt, the standing chain links' resting angle, and the fob bar's height were all set by guess-and-screenshot; objects that float 5mm or sink 5mm read fine in a viewport and wrong in a final render.
- **P11 — `diff_since(checkpoint|action_id)` — feel what changed.** Per-object geometry delta narrative since a history point: moved/rotated (mm/deg), deformed (max displacement, where), added/deleted. Closes the loop on sculpt brushes, which today are open-loop — after `sculpt_crease` the only feedback is a screenshot; want `"head: deformed — crease deepened 4mm along 0.3m path on left cheek"`. Pairs with the history log + `save_design` checkpoints that already exist.
- **P12 — `audit_asset(group)` — game-readiness audit.** One report per delivery: objects with no material slot, per-object tri counts flagged against projected screen size (`"crystal: 9.2k tris for 8% of frame — overbuilt"`), unapplied scale/rotation, loose verts. The "is this actually game-ready" judgment is currently vibes; every check in it is countable.
