...
# ─────────────────────────────────────────────────────────────────────────────
# G138 — set_textured_material accepts a LIST target (multi-object, one material).
# (The server routes "Mug,Handle" through _targets() → a list; here we drive the
# extension handler directly with the list it now receives.)
# ─────────────────────────────────────────────────────────────────────────────
print("\nG138 — textured material shades a multi-object target with ONE material\n")

clean()
make_box("Mug", 0.0, 0.1, 0.1, 0.12)
make_box("Handle", 0.0, 0.03, 0.06, 0.03)
bpy.data.objects["Handle"].location = (0.13, 0, 0)
# A real 4×4 image on disk so the node graph's image load has a file to read.
img = bpy.data.images.new("g138_diff", 4, 4)
diff_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_g138_diff.png")
img.filepath_raw = diff_path
img.file_format = 'PNG'
img.save()
bpy.data.images.remove(img)

mat_count_before = len(bpy.data.materials)
r = run("set_textured_material", target=["Mug", "Handle"],
        maps={"diffuse": diff_path}, material_name="vessel_tex")
check("textured material on a list target succeeds", r.get("success") is True, str(r.get("error")))
check("BOTH objects were shaded in one call",
      sorted(r.get("assigned_to", [])) == ["Handle", "Mug"], str(r.get("assigned_to")))
check("exactly ONE material datablock was created (not one per object)",
      len(bpy.data.materials) - mat_count_before == 1,
      f"created={len(bpy.data.materials) - mat_count_before}")
m_mug = bpy.data.objects["Mug"].data.materials
m_handle = bpy.data.objects["Handle"].data.materials
check("Mug and Handle share the same material datablock",
      len(m_mug) and len(m_handle) and m_mug[0] == m_handle[0],
      f"mug={m_mug[0].name if len(m_mug) else None}, handle={m_handle[0].name if len(m_handle) else None}")
try:
    os.remove(diff_path)
except OSError:
    pass
...
# ─────────────────────────────────────────────────────────────────────────────
# G141 — physical_size derives the box-projection Mapping scale from the object's
# measured world scale (world_scale / physical_size), instead of a unitless guess.
# ─────────────────────────────────────────────────────────────────────────────
print("\nG141 — physical_size makes texture scale a derived real-world quantity\n")

clean()
# A donut-sized part (≈9cm) at object scale 1.0, and a table-sized one at scale 4.0. One
# physical_size must yield DIFFERENT mapping scales so the grain reads the same physical
# size on both — exactly what a single unitless `scale` could never do.
make_box("Donut", 0.0, 0.045, 0.045, 0.02)            # world scale 1.0
big = make_box("Table", 0.0, 0.05, 0.05, 0.02)
big.scale = (4.0, 4.0, 4.0)                            # unapplied object scale
bpy.context.view_layer.update()

g141_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_g141_diff.png")
img = bpy.data.images.new("g141_diff", 4, 4)
img.filepath_raw = g141_path
img.file_format = 'PNG'
img.save()
bpy.data.images.remove(img)

PHYS = 0.05  # one texture tile = 5cm in the real world


def mapping_scale_of(obj):
...
r = run("set_textured_material", target="Donut", maps={"diffuse": g141_path},
        physical_size=PHYS, material_name="donut_phys")
check("textured with physical_size succeeds", r.get("success") is True, str(r.get("error")))
check("result echoes physical_size", abs((r.get("physical_size") or 0) - PHYS) < 1e-9, str(r.get("physical_size")))
ms = mapping_scale_of(bpy.data.objects["Donut"])
check("scale-1 object: mapping scale = world_scale/physical_size (=20)",
      ms and all(abs(c - (1.0 / PHYS)) < 1e-3 for c in ms[:3]), str(ms))

r = run("set_textured_material", target="Table", maps={"diffuse": g141_path},
        physical_size=PHYS, material_name="table_phys")
ms2 = mapping_scale_of(bpy.data.objects["Table"])
check("scale-4 object: mapping scale tracks world scale (=80, 4× the donut's)",
      ms2 and all(abs(c - (4.0 / PHYS)) < 1e-2 for c in ms2[:3]), str(ms2))
