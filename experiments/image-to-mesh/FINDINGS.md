...
| 15 | Better boundary detection (cardinal-scan outline) | ✅ materially better. `rung15_boundary.py`: a point is INTERIOR iff its column has a point above+below AND its row has a point left+right; missing any → it's on the cloud OUTLINE → boundary. Then loop the boundary (each boundary pt → 2 nearest boundary pts) and force interior valence-4, with a final de-tri. vs rung14's ink-gap boundary (152 pts, 49%, inflated by under-extraction), the cardinal scan gives **82 boundary pts (26%, realistic)** — so far fewer interior verts are wrongly exempted, and forcing actually fires: **head_v10.obj = 0 tris, 134 quads (was 107), 64% coverage (was 58%), interior@4 120/229.** Outer silhouette closes cleanly; eyes survive (no-cross/no-tri gates stop the fill bridging holes). Limit: catches the OUTER outline only (hole rims read interior); coverage still 64% — the rest needs the missing vertices a denser-or-sparser regen would provide. |
...
- `rung15_boundary.py` — cardinal-scan outline boundary (26%, vs 49% ink-gap) +
  boundary loop + forced interior + final de-tri -> head_v10.obj (0 tris, 134 quads)
