...
| 10 | Can we build a mesh from the AUTHORED edges (no Delaunay)? | 🟡 machinery works, recall-gated — `rung10_mesh_from_graph.py` recovers faces from the rung9 edge graph by a planar half-edge traversal (clockwise-most turn at each node; keep 3–5-gons, drop the outer face + big feature holes so eyes/mouth stay open). On `v3_front`: 282 nodes / 383 edges → **31 faces (21 quads, 7 tris, 3 pentagons)**, `head_v4.obj`. The 21 quads are REAL authored-edge quads (zero Delaunay) — but most of the surface stays open because **a quad needs all four edges and rung9 recall is ~2.7/4**, so each missing edge pops a cycle open. Verdict: face recovery is ready; **edge recall is the gate.** Visualized in `rung10_faces.png` (closed quads scattered over a complete wire grid). |
| 9 | Can we extract the REAL quad edge graph (not Delaunay soup) from one view? | 🟡 yes, mostly — `rung9_trace.py` inverts rung3: nodes are the red dots, connectivity is LOCAL link-prediction. Per dot, ring-sample the directions ink leaves it, walk each direction's cone nearest-first, accept the first dot whose **chord validates against the ink**. Curve-tolerant: the validation tube WIDENS toward mid-span (`base + curve·L`), so Grok's bowed edges pass while dense short edges stay tight. On `v3_front`: 282 nodes, **383 edges, avg degree 2.7** (~2/3 of a full quad grid), **precision ~perfect** (every accepted edge rides a wire: median 1.4px, max 7.5px). **Succeeds in the eye/nose/mouth feature regions where rung3 skeletonize failed.** Length-sanity flags the 13 genuinely-curved long spans for review. Recall gap = the 2nd grid direction where the ring merged close directions (dense eye/nose) or a bow exceeded the tube — tunable, not fundamental. No turnkey library; built from numpy+scipy (distance transform + kdtree). |
...
- `rung10_mesh_from_graph.py` — mesh from the AUTHORED edges (no Delaunay):
  planar half-edge face traversal of the rung9 graph -> head_v4.obj; recall-gated
